...
Do not assume that negative-prompt syntax or a dedicated exclusion field is supported. Verify the active Seedance surface before relying on any syntax. State the required outcome plainly and treat explicit platform fields as provider-specific.
...
Use an output contract when it makes the creative result clearer. Do not claim that one prompt form is universally supported or more effective.
